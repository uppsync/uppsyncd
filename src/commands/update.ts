...
export async function update() {
    ...
    try {
        const res = await fetch("https://api.github.com/repos/uppsync/uppsyncd/releases/latest");
        ...
        if (process.platform === "linux") {
            ...
            const isDebian = (await $`dpkg -S ${currentPath}`.quiet().nothrow()).exitCode === 0;
            if (isDebian) {
                console.log("\nThis installation is managed by apt/dpkg.");
                console.log("Please use: sudo apt-get update && sudo apt-get install uppsyncd");
                return;
            }

            // Check if managed by rpm (RHEL/CentOS)
            const isRpm = (await $`rpm -qf ${currentPath}`.quiet().nothrow()).exitCode === 0;
            if (isRpm) {
        ...
        if (process.platform === "win32") {
            ...
            try {
                ...
                if (e.exitCode === 1) { // Permission denied usually
